...
function AppContent() {
  ...
  if (dogs.length === 0 && screen !== 'onboarding') return (
  ...
  const handleChat = (receiverId) => {
    setChatReceiver(receiverId); setTab('chat'); setScreen('main'); setSelectedArea(null)
  }
  ...
  const renderMain = () => {
    ...
    switch (tab) {
      case 'home': return <HomePage onAreaSelect={handleAreaSelect} onTabChange={handleTabChange} onEventSelect={handleEventSelect} onChat={handleChat} />
      ...
      case 'profile': return <ProfilePage onAddDog={()=>setScreen('onboarding')} onAdminPanel={()=>setScreen('admin')} onChat={handleChat} />
      default: return <HomePage onAreaSelect={handleAreaSelect} onTabChange={handleTabChange} onEventSelect={handleEventSelect} onChat={handleChat} />
    }
  }
      ...
      <div className="w-full bg-white shadow-2xl overflow-hidden flex flex-col"
        ...
        <div className="hidden sm:flex bg-white px-6 py-2 items-center justify-between text-xs font-semibold border-b border-gray-50 flex-shrink-0">
          ...
          <div className="flex items-center gap-1">
            <svg className="w-4 h-3" fill="currentColor" viewBox="0 0 16 12"><path d="M1 8h2v3H1zM5 6h2v5H5zM9 4h2v7H9zM13 2h2v9h-2z"/></svg>
            <div className="w-6 h-3 border border-gray-900 rounded-sm flex items-center px-0.5"><div className="w-4/5 h-full bg-gray-900 rounded-sm"/></div>
          </div>
        </div>
